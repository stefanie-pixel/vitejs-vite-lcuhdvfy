...
function LoginScreen({ onLogin, onUser }) {
  ...
  const tryLogin = () => { if (onLogin(pin)) { setErr(false); } else { setErr(true); setPin(""); setTimeout(() => setErr(false), 2000); } };
  return (
      ...
      <div style={{ width: "100%", maxWidth: 380, background: "var(--bg2)", border: "0.5px solid var(--border)", borderRadius: 16, padding: 32 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(43,191,191,0.12)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>
            <i className="ti ti-shield-check" style={{ fontSize: 26, color: "var(--teal)" }} />
          </div>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ fontSize: 12, color: "var(--text3)", display: "block", marginBottom: 6 }}>Admin-PIN</label>
          <div style={{ position: "relative" }}>
            <input type={show ? "text" : "password"} value={pin} onChange={e => setPin(e.target.value)} onKeyDown={e => e.key === "Enter" && tryLogin()} placeholder="PIN eingeben..." autoFocus />
            <button onClick={() => setShow(!show)} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "var(--text3)", padding: 4 }}>
              <i className={`ti ${show ? "ti-eye-off" : "ti-eye"}`} style={{ fontSize: 14 }} />
            </button>
          </div>
          {err && <p style={{ fontSize: 12, color: "var(--red)", margin: "6px 0 0" }}>Falscher PIN</p>}
        </div>
        <button onClick={tryLogin} style={{ width: "100%", background: "rgba(43,191,191,0.15)", borderColor: "var(--teal)", color: "var(--teal)", padding: "10px", fontSize: 14, fontWeight: 500, marginBottom: 10 }}>
          <i className="ti ti-login" style={{ marginRight: 6 }} />Als Admin anmelden
        </button>
        <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "14px 0" }}>
          <div style={{ flex: 1, height: "0.5px", background: "var(--border)" }} /><span style={{ fontSize: 12, color: "var(--text3)" }}>oder</span><div style={{ flex: 1, height: "0.5px", background: "var(--border)" }} />
        </div>
        <button onClick={onUser} style={{ width: "100%", padding: "10px", fontSize: 14, color: "var(--text2)" }}>
          <i className="ti ti-user" style={{ marginRight: 6 }} />Als QM / Teamleiter öffnen
        </button>
        <div style={{ marginTop: 18, padding: "10px 12px", background: "var(--bg3)", borderRadius: 8, border: "0.5px solid var(--border)" }}>
          <p style={{ fontSize: 11, color: "var(--text3)", margin: 0, lineHeight: 1.5 }}>
            <i className="ti ti-info-circle" style={{ marginRight: 4, color: "var(--blue)" }} />
            <strong style={{ color: "var(--text2)" }}>Admin:</strong> Vollzugriff – Projekte, Handbuch, Steckbrief, KPIs.<br />
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {handbook.map((s, i) => {
          const isOpen = open === i;
          return (
            <div key={s.id} style={{ border: `0.5px solid ${isOpen ? CM[s.color] || "var(--teal)" : "var(--border)"}`, borderRadius: 10, overflow: "hidden" }}>
                ...
                <div style={{ padding: "10px 14px 14px 52px", background: "var(--bg2)" }}>
                      ...
                      <li key={j} style={{ fontSize: 13, color: "var(--text2)", lineHeight: 1.6 }}>
                        {(editMode && isAdmin) ? (
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            ...
                            <button onClick={() => rmStep(s.id, j)} style={{ background: "none", border: "none", color: "var(--red)", padding: "2px 6px", flexShrink: 0 }}><i className="ti ti-x" style={{ fontSize: 11 }} /></button>
                          </div>
                        ) : step}
                      </li>
                    ))}
                  ...
                  {(editMode && isAdmin) && (
                    <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                      <input value={newStep} onChange={e => setNewStep(e.target.value)} onKeyDown={e => { if (e.key === "Enter") addStep(s.id); }} placeholder="Neuer Schritt..." />
                      <button onClick={() => addStep(s.id)}>+ Schritt</button>
                    </div>
                  )}
                </div>
              )}
      ...
      {(editMode && isAdmin) && (
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <input value={newPhase} onChange={e => setNewPhase(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && newPhase.trim()) { setHandbook([...handbook, { id: Date.now(), phase: newPhase.trim(), icon: "ti-star", color: "teal", steps: [] }]); setNewPhase(""); } }} placeholder="Neue Phase..." />
          <button onClick={() => { if (newPhase.trim()) { setHandbook([...handbook, { id: Date.now(), phase: newPhase.trim(), icon: "ti-star", color: "teal", steps: [] }]); setNewPhase(""); } }}>+ Phase</button>
        </div>
      )}
...
function MonitoringSection({ saved, setSaved, projects }) {
  ...
  const missing = [];
  if (!agentName) missing.push("Agent/in");
  if (!qm) missing.push("QM / TL");
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: activeTypes.length === 1 ? "1fr" : activeTypes.length === 2 ? "1fr 1fr" : "repeat(auto-fit,minmax(280px,1fr))", gap: 14, marginBottom: 14 }}>
        {activeTypes.map(tk => {
          ...
          const sc = scores[tk] || {}; const pct = calcPct(tk);
          return (
            <div key={tk} style={{ background: "var(--bg3)", border: `0.5px solid ${CM[td?.color] || "var(--teal)"}`, borderRadius: 10, padding: "14px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
...
// ── COACHING ───────────────────────────────────────────────────────────────
const SMART_CONFIG = [
  { k: "smart_s", l: "S", q: "Spezifisch — Was genau soll verbessert werden?", sub: "Welches konkrete Verhalten wurde beobachtet?", ph: "z.B. Servicefragen werden aktuell nur in 2 von 5 Calls gestellt. Ziel: mind. 4 von 5.", c: "var(--teal)", bc: "rgba(43,191,191,0.2)" },
  { k: "smart_m", l: "M", q: "Messbar — Woran erkennen wir den Erfolg?", sub: "Welche Kennzahl / welchen Wert wollen wir erreichen?", ph: "z.B. Bewertung Servicefragen steigt von 40 % auf mind. 80 % im nächsten Monitoring.", c: "var(--blue)", bc: "rgba(46,134,171,0.2)" },
  { k: "smart_a", l: "A", q: "Attraktiv — Warum ist dieses Ziel wichtig?", sub: "Was motiviert den Agenten? Welchen Mehrwert hat das Ziel?", ph: "z.B. Höhere CSAT, bessere Gesamtbewertung — Grundlage für Entwicklungsgespräch Q3.", c: "#c49fe8", bc: "rgba(123,79,166,0.2)" },
  { k: "smart_r", l: "R", q: "Realistisch — Ist das Ziel erreichbar?", sub: "Welche Unterstützung / Ressourcen stehen zur Verfügung?", ph: "z.B. Ja — Servicefragen wurden in Schulung behandelt, Fragekatalog liegt vor.", c: "var(--orange)", bc: "rgba(245,166,35,0.2)" },
  { k: "smart_t", l: "T", q: "Terminiert — Bis wann soll das Ziel erreicht sein?", sub: "Wann ist das nächste Follow-up / Monitoring geplant?", ph: "z.B. Bis KW 28 — nächstes Monitoring in 2 Wochen. Follow-up am [Datum].", c: "var(--green)", bc: "rgba(43,191,140,0.2)" },
];
...
function CoachingSection({ project, entries, setEntries, projects }) {
  ...
  if (!form.goal) missing.push("Hauptziel");
  if (!smartDone) missing.push("SMART vollständig");
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ display: "flex", gap: 3 }}>
              {SMART_CONFIG.map(({ k, l, c }) => <span key={l} style={{ width: 26, height: 26, borderRadius: 7, background: form[k] ? "rgba(43,191,191,0.2)" : "var(--bg4)", border: `1px solid ${form[k] ? c : "var(--border)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: form[k] ? c : "var(--text3)", transition: "all 0.2s" }}>{l}</span>)}
            </div>
          ...
          <span style={{ fontSize: 11, color: smartDone ? "var(--green)" : "var(--text3)" }}>{SMART_CONFIG.filter(x => form[x.k]).length}/5</span>
        </div>
        {SMART_CONFIG.map(({ k, l, q, sub, ph, c, bc }) => (
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, color: "var(--text3)", display: "block", marginBottom: 4 }}>Dokumentation <span style={{ fontSize: 11, color: "var(--text3)", fontStyle: "italic" }}>(Beobachtungen, Call-IDs, Ticket-Links)</span></label>
        <textarea value={form.doc} onChange={e => upd("doc", e.target.value)} rows={3} placeholder="z.B. Call ID 2460398 – Servicefragen nicht gestellt. Agent zeigt gute Empathie." />
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10 }}>
        {missing.length > 0 && <span style={{ fontSize: 12, color: "var(--text3)" }}>Pflichtfelder: {missing.map((m, i) => <span key={m} style={{ color: "var(--orange)" }}>{m}{i < missing.length - 1 ? ", " : ""}</span>)}</span>}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {entries.map(e => (
              <div key={e.id} style={{ background: "var(--bg3)", border: "0.5px solid var(--border)", borderRadius: 10, padding: "12px 14px" }}>
                ...
                {SMART_CONFIG.some(x => e[x.k]) && (
                  <div style={{ paddingLeft: 19, display: "flex", flexDirection: "column", gap: 3, marginBottom: e.doc ? 6 : 0 }}>
                    {SMART_CONFIG.filter(x => e[x.k]).map(({ k, l, c }) => (
                      <div key={k} style={{ display: "flex", gap: 6 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: c, width: 12, flexShrink: 0, marginTop: 2 }}>{l}</span>
                        <span style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.5 }}>{e[k]}</span>
                      </div>
                    ))}
                  </div>
                )}
...
function AgentModal({ name, monitorings, coachings, projects, onClose }) {
  const allMon = Object.entries(monitorings).flatMap(([pid, items]) => (items || []).filter(m => m.agentName === name).map(m => ({ ...m, pName: projects.find(p => p.id == pid)?.name || m.pName || "" }))).sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0));
  const allCoa = Object.entries(coachings).flatMap(([pid, items]) => (items || []).filter(e => e.agent === name).map(e => ({ ...e, pName: projects.find(p => p.id == pid)?.name || e.pName || "" }))).sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0));
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: "var(--bg2)", border: "0.5px solid var(--border2)", borderRadius: 14, width: "100%", maxWidth: 860, maxHeight: "90vh", overflow: "auto", padding: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            {allMon.length === 0 ? <p style={{ fontSize: 13, color: "var(--text3)" }}>Keine</p> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {allMon.map(m => (
                  <div key={m.id} style={{ background: "var(--bg3)", border: "0.5px solid var(--border)", borderRadius: 8, padding: "9px 12px" }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {(m.activeTypes || []).map(t => m.typePcts?.[t] != null && <Badge key={t} color={m.typePcts[t] >= 90 ? "green" : m.typePcts[t] >= 75 ? "orange" : "red"} sm>{DEFAULT_TYPES[t]?.label}: {m.typePcts[t]}%</Badge>)}
                      {m.overall != null && <Badge color={m.overall >= 90 ? "green" : m.overall >= 75 ? "orange" : "red"}>Ø {m.overall}%</Badge>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 500, color: "var(--teal)", marginBottom: 8 }}>Coachings & SMART-Ziele</p>
            {allCoa.length === 0 ? <p style={{ fontSize: 13, color: "var(--text3)" }}>Keine</p> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {allCoa.map(e => (
...
function OverviewSection({ project, monitorings, coachings, projects }) {
  ...
  const topicCounts = {};
  coa.filter(e => inM(e.date)).forEach(e => { if (e.topic) topicCounts[e.topic] = (topicCounts[e.topic] || 0) + 1; });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {allAgents.length > 0 && (
        <Card>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {allAgents.map(name => {
              const aMon = mon.filter(m => m.agentName === name);
              const aCoa = coa.filter(e => e.agent === name);
              const last = aMon[0];
              return (
                <button key={name} onClick={() => setSelAgent(name)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", background: "var(--bg3)", border: "0.5px solid var(--border)", borderRadius: 10, cursor: "pointer", textAlign: "left" }}>
                  ...
                  <div><p style={{ fontSize: 13, fontWeight: 500, color: "var(--text)", margin: 0 }}>{name}</p><p style={{ fontSize: 11, color: "var(--text3)", margin: 0 }}>{aMon.length} Mon · {aCoa.length} Coa{last?.overall != null ? ` · Ø ${last.overall}%` : ""}</p></div>
                  <i className="ti ti-chevron-right" style={{ fontSize: 13, color: "var(--text3)", marginLeft: 4 }} />
                </button>
              );
            })}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {recentMon.map(m => (
              <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", background: "var(--bg3)", borderRadius: 7 }}>
                ...
                <span style={{ fontSize: 11, color: "var(--text3)" }}>{m.date}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Card>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <p style={{ fontSize: 13, fontWeight: 500, color: "var(--teal)", margin: 0 }}>AG-Reporting <span style={{ fontSize: 11, color: "var(--text3)", fontWeight: 400 }}>automatisch</span></p>
          <button onClick={() => { navigator.clipboard.writeText(report); setCopied(true); setTimeout(() => setCopied(false), 2000); }} style={{ background: copied ? "rgba(43,191,191,0.15)" : "var(--bg3)", borderColor: copied ? "var(--teal)" : "var(--border)", color: copied ? "var(--teal)" : "var(--text2)", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}><i className={`ti ${copied ? "ti-check" : "ti-copy"}`} />{copied ? "Kopiert!" : "Kopieren"}</button>
        </div>
        <textarea readOnly value={report} rows={4} style={{ fontSize: 13, lineHeight: 1.6, resize: "none", color: "var(--text2)" }} />
      </Card>
    </div>
  );
}
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr key={row} style={{ background: ri % 2 === 0 ? "var(--bg4)" : "var(--bg3)" }}>
                ...
                {view === "monat" ? MONTHS.map(m => {
                  const val = gv(row, m); const ziel = gz(row);
                  return <td key={m} style={{ padding: "3px 4px", borderBottom: "0.5px solid rgba(255,255,255,0.04)", textAlign: "center" }}>
                    {isAdmin ? <input value={val} onChange={e => sv(row, m, e.target.value)} style={{ width: 46, padding: "2px 4px", fontSize: 11, textAlign: "center", background: "transparent", border: "0.5px solid transparent", color: pColor(val, ziel) }} onFocus={e => e.target.style.borderColor = "var(--teal)"} onBlur={e => e.target.style.borderColor = "transparent"} /> : <span style={{ fontSize: 11, color: pColor(val, ziel) }}>{val || "–"}</span>}
                  </td>;
...
function SteckbriefSection({ project, onUpdate, isAdmin }) {
  ...
  const Sec = ({ title, icon, color, children }) => (
    <div style={{ background: "var(--bg3)", border: `0.5px solid ${CM[color] || "var(--border)"}`, borderRadius: 10, padding: "14px 16px", marginBottom: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        <span style={{ width: 28, height: 28, borderRadius: 7, background: BM[color] || "var(--bg4)", display: "flex", alignItems: "center", justifyContent: "center" }}><i className={`ti ${icon}`} style={{ fontSize: 14, color: CM[color] || "var(--teal)" }} /></span>
        <p style={{ fontSize: 13, fontWeight: 500, color: CM[color] || "var(--teal)", margin: 0 }}>{title}</p>
      </div>
      {children}
    </div>
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["info", "kpi"].map(t => (
            <button key={t} onClick={() => setSbTab(t)} style={{ fontSize: 12, padding: "5px 14px", background: sbTab === t ? "rgba(43,191,191,0.15)" : "var(--bg3)", borderColor: sbTab === t ? "var(--teal)" : "var(--border)", color: sbTab === t ? "var(--teal)" : "var(--text2)" }}>
      ...
      {sbTab === "info" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 14 }}>
            {[{ label: "Kampagnen-ID", fk: "kampagnenId", icon: "ti-hash", color: "teal" }, { label: "Stand", fk: "stand", icon: "ti-calendar", color: "blue" }, { label: "Projekttyp", fk: "projekttyp", icon: "ti-tag", color: "purple" }, { label: "Servicezeiten", fk: "servicezeiten", icon: "ti-clock", color: "orange" }].map(({ label, fk, icon, color }) => (
              <div key={fk} style={{ background: "var(--bg3)", border: `0.5px solid ${CM[color] || "var(--border)"}`, borderRadius: 9, padding: "11px 13px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <Sec title="KPIs, Forecast & Stoßzeiten" icon="ti-chart-bar" color="orange"><F label="Wichtige KPIs" fk="kpis" multi ph="z.B. AHT < 6 Min, FCR > 90 %..." /><F label="Forecast & Auslastung" fk="forecast" multi /><F label="Stoßzeiten" fk="stoszeiten" ph="z.B. 10:00–12:00" /><F label="Blending" fk="blending" ph="z.B. Check24 Möbel" /></Sec>
...
// ── SETTINGS ───────────────────────────────────────────────────────────────
function TypeCard({ tk, ep, updP, newCrit, setNewCrit, getBase, updBase }) {
  const [editCrit, setEditCrit] = useState(false);
  const td = DEFAULT_TYPES[tk];
  const tc = ep.monTypes?.[tk] || { active: false, extra: [] };
  const base = getBase(tk);
  return (
    <div style={{ background: "var(--bg4)", borderRadius: 10, padding: "12px 14px", border: `0.5px solid ${tc.active ? CM[td?.color] || "var(--teal)" : "var(--border)"}` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}><i className={`ti ${td?.icon || "ti-file"}`} style={{ fontSize: 15, color: tc.active ? CM[td?.color] || "var(--teal)" : "var(--text3)" }} /><span style={{ fontSize: 13, fontWeight: 500, color: tc.active ? "var(--text)" : "var(--text3)" }}>{td?.label}</span></div>
        <button onClick={() => updP(ep.id, { monTypes: { ...ep.monTypes, [tk]: { ...tc, active: !tc.active } } })} style={{ padding: "4px 12px", fontSize: 12, background: tc.active ? "rgba(43,191,191,0.15)" : "var(--bg3)", borderColor: tc.active ? "var(--teal)" : "var(--border)", color: tc.active ? "var(--teal)" : "var(--text3)" }}>{tc.active ? "✓ Aktiv" : "Inaktiv"}</button>
      </div>
      {tc.active && (
        <>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 11, color: "var(--text3)" }}>Kriterien ({base.length})</span>
            <button onClick={() => setEditCrit(!editCrit)} style={{ fontSize: 10, padding: "2px 8px", background: editCrit ? "rgba(43,191,191,0.15)" : "var(--bg3)", borderColor: editCrit ? "var(--teal)" : "var(--border)", color: editCrit ? "var(--teal)" : "var(--text3)" }}>{editCrit ? "✓ Fertig" : "✎ Edit"}</button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 6 }}>
            {base.map((c, idx) => (
              <div key={c.key} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                {editCrit ? <><input value={c.label} onChange={e => { const b = [...base]; b[idx] = { ...b[idx], label: e.target.value }; updBase(tk, b); }} style={{ flex: 1, fontSize: 11, padding: "2px 7px" }} /><button onClick={() => updBase(tk, base.filter((_, j) => j !== idx))} style={{ background: "none", border: "none", color: "var(--red)", padding: "1px 4px", flexShrink: 0 }}><i className="ti ti-x" style={{ fontSize: 10 }} /></button></> : <span style={{ fontSize: 11, color: "var(--text2)", padding: "2px 0" }}>· {c.label}</span>}
              </div>
            ))}
            {editCrit && <div style={{ display: "flex", gap: 5, marginTop: 4 }}>
              <input value={newCrit[tk] || ""} onChange={e => setNewCrit({ ...newCrit, [tk]: e.target.value })} onKeyDown={e => { if (e.key === "Enter" && (newCrit[tk] || "").trim()) { updBase(tk, [...base, { key: `c_${Date.now()}`, label: (newCrit[tk] || "").trim() }]); setNewCrit({ ...newCrit, [tk]: "" }); } }} placeholder="Neues Kriterium..." style={{ flex: 1, fontSize: 11, padding: "2px 7px" }} />
              <button onClick={() => { if ((newCrit[tk] || "").trim()) { updBase(tk, [...base, { key: `c_${Date.now()}`, label: (newCrit[tk] || "").trim() }]); setNewCrit({ ...newCrit, [tk]: "" }); } }} style={{ padding: "2px 8px", fontSize: 11, flexShrink: 0 }}>+</button>
            </div>}
          </div>
          {(tc.extra || []).length > 0 && <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 6, paddingTop: 6, borderTop: "0.5px solid var(--border)" }}>{(tc.extra || []).map(c => <div key={c.key} style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--bg3)", borderRadius: 5, padding: "3px 7px" }}><span style={{ flex: 1, fontSize: 11, color: "var(--teal)" }}>+ {c.label}</span><button onClick={() => updP(ep.id, { monTypes: { ...ep.monTypes, [tk]: { ...tc, extra: (tc.extra || []).filter(x => x.key !== c.key) } } })} style={{ background: "none", border: "none", color: "var(--red)", padding: "1px 3px" }}><i className="ti ti-x" style={{ fontSize: 10 }} /></button></div>)}</div>}
          <div style={{ display: "flex", gap: 5, paddingTop: 4, borderTop: "0.5px solid var(--border)" }}>
            <input value={newCrit[`extra_${tk}`] || ""} onChange={e => setNewCrit({ ...newCrit, [`extra_${tk}`]: e.target.value })} onKeyDown={e => { if (e.key === "Enter" && (newCrit[`extra_${tk}`] || "").trim()) { updP(ep.id, { monTypes: { ...ep.monTypes, [tk]: { ...tc, extra: [...(tc.extra || []), { key: `e_${Date.now()}`, label: (newCrit[`extra_${tk}`] || "").trim() }] } } }); setNewCrit({ ...newCrit, [`extra_${tk}`]: "" }); } }} placeholder="+ Zusatzkriterium..." style={{ fontSize: 11, padding: "3px 8px" }} />
            <button onClick={() => { if ((newCrit[`extra_${tk}`] || "").trim()) { updP(ep.id, { monTypes: { ...ep.monTypes, [tk]: { ...tc, extra: [...(tc.extra || []), { key: `e_${Date.now()}`, label: (newCrit[`extra_${tk}`] || "").trim() }] } } }); setNewCrit({ ...newCrit, [`extra_${tk}`]: "" }); } }} style={{ padding: "3px 9px", fontSize: 11, flexShrink: 0 }}>+</button>
          </div>
        </>
      )}
    </div>
      ...
      {!editing ? (
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
            {projects.map(p => {
              const at = Object.entries(p.monTypes || {}).filter(([, c]) => c.active).map(([k]) => DEFAULT_TYPES[k]?.label || k);
              return (
        ...
        <>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {["call", "ticket", "mail", "chat"].map(tk => <TypeCard key={tk} tk={tk} ep={ep} updP={updP} newCrit={newCrit} setNewCrit={setNewCrit} getBase={getBase} updBase={updBase} />)}
            </div>
          </div>
        </>
      ) : null}
...
export default function QMTool() {
  ...
  const tabs = [
    ...
    { key: "steckbrief", label: "Projektsteckbrief" },
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", paddingBottom: 60 }}>
      ...
      <div style={{ background: "var(--bg2)", borderBottom: "0.5px solid var(--border)", padding: "13px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={logout} style={{ background: "none", border: "none", color: "var(--text3)", padding: "3px 6px" }} title="Abmelden"><i className="ti ti-logout" style={{ fontSize: 14 }} /></button>
